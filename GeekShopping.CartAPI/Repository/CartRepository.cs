	...
	public class CartRepository : ICartRepository
	{
        ...
        public async Task<CartVO> SaveOrUpdateCart(CartVO vo)
        {
            try
            {
                ...
                var category = await _context.Categories.Where(p => p.Id == cart.CartDetails.FirstOrDefault().Product.CategoryId).FirstOrDefaultAsync();

                if (category == null)
                {
                    category = _mapper.Map<Category>(vo.CartDetails.FirstOrDefault().Category);
                    _context.Categories.Add(_mapper.Map<Category>(vo.CartDetails.FirstOrDefault().Category));
                    await _context.SaveChangesAsync();
                }

                var product = await _context.Products.Where(p => p.Id == vo.CartDetails.FirstOrDefault().ProductId).FirstOrDefaultAsync();
                ...
                {
                    var cartDetail = await _context.CartDetails.AsNoTracking().FirstOrDefaultAsync(x => x.ProductId == cart.CartDetails.FirstOrDefault().ProductId
